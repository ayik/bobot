...
✞ Enter your whatsapp number with the country code e.g;  255763755563 then tap submit 
✞Goodchild Md owner , *goodchild* ,will send you the code immediately.Copy the code and whatsapp will bring a notification 
✞Tap on the notification and paste the code that Goodchild md owner sent you.
✞ After a successful login Goodchild Md owner 😂 Goochild will send you a *session id* .some will wonder where the hell is that session ,,,...It is just right at your inbox /dm {your own number for whatsappp} 😂
✞Copy the Session id {the one with unreadable codes and long copy it and send it to your deployer or deploy
     *ʜᴏᴡ ᴛᴏ ᴅᴇᴘʟᴏʏ 𝐆𝐎𝐎𝐃𝐂𝐇𝐈𝐋𝐃 𝐌𝐃*
✔ Now check on Alpha repository in github fork and give a star to this repository before doing anything 🌟or else Goodchild Md owner won't allow you to deploy his bot💀.
...
╰────────────────────☆`;
let menuMsg = `
     𝐑𝐞𝐠𝐚𝐫𝐝𝐬 *𝐆𝐎𝐎𝐃𝐂𝐇𝐈𝐋𝐃 𝐓𝐄𝐀𝐌*`;
   var lien = mybotpic();

   if (lien.match(/\.(mp4|gif)$/i)) {
    try {
        zk.sendMessage(dest, { video: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *Goodchld*, déveloper Keith Tech" , gifPlayback : true }, { quoted: ms });
    }
    catch (e) {
